...
export function registerReceiptRoutes(app: FastifyInstance, pool: Pool) {
  app.get("/api/v1/organizations/:id/receipts", async (request) => {
    ...
    const rows = (
      ...
      from receipts r join chain_events e on e.id=r.event_id where r.organization_id=$1
  ...
  app.post("/api/v1/organizations/:id/receipt-exports", async (request, reply) => {
    const { id } = idParams(request),
    ...
    const result = await mutate(
      ...
      async (c) => {
        ...
        where r.organization_id=$1 and r.status='FINAL' and e.finality_state='FINAL'
        ...
        order by r.created_at,r.id limit 1001`,
            [id, filters.category ?? null, filters.from ?? null, filters.to ?? null],
          )
        ).rows;
        if (rows.length > 1000)
        ...
        const records = rows.map((r) => {
          if (
            r.asset !== ARC_USDC ||
            (r.bounty_id &&
              (r.bounty_organization !== id ||
                (r.category !== "BUDGET_ALLOCATION" && r.contract_address !== r.bounty_escrow))) ||
  ...
  async function owned(id: string, actor: { id: string; displayName: string }) {
    ...
    await member(pool, actor, row.organization_id, ["OWNER", "TREASURY"]);
    return row;
  }
  ...
  app.get("/api/v1/organizations/:id/receipt-exports", async (request) => {
    const { id } = idParams(request);
    await member(pool, request.actor, id, ["OWNER", "TREASURY"]);
    const rows = (
      await pool.query(
        "select * from receipt_exports where organization_id=$1 and requested_by=$2 order by created_at desc limit 20",
        [id, request.actor.id],
      )
    ).rows;
