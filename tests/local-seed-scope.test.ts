...
const local = {
  ...
  LOCAL_SEED_DATABASE_URL: "postgres://test:test@127.0.0.1:5433/proofofhack_seed_demo",
};
it("Requires explicit local chain and separate database scope", () => {
  expect(localSeedScope(local).name).toBe("proofofhack_seed_demo");
...
it("Rejects shared databases, remote hosts, connection overrides, and SQL identifiers", () => {
  for (const url of [
    "postgres://test@127.0.0.1:5433/proofofhack",
    "postgres://test@example.com:5433/proofofhack_seed_demo",
    "postgres://test@127.0.0.1:5433/proofofhack_seed_demo?host=example.com",
    "postgres://test@127.0.0.1:5433/proofofhack_seed_demo%22",
    "postgres://test@127.0.0.1:5433/proofofhack_seed_demo#x",
  ])
